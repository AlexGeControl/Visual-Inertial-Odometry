...
string file_1 = "../data/02-direct-method/left.png";  // first image
string file_2 = "../data/02-direct-method/right.png";  // second image
string file_disparity = "../data/02-direct-method/disparity.png";
...
struct Values {
    void Reset(void) { v.clear(); }
    void Add(double v) { this->v.push_back(v); }

    void GetStats(double &mean, double &std) {
        // calculate mean:
...
struct CorrCoeff {
    ...
    double GetCoeff(void) {
        double mu_est, std_est;
        estimated.GetStats(mu_est, std_est);

        double mu_ref, std_ref;
        reference.GetStats(mu_ref, std_ref);

        const size_t N = estimated.v.size();
        double accum = 0.0;
        for (size_t i = 0; i < N; ++i) {
            accum += (estimated.v.at(i) - mu_est) * (reference.v.at(i) - mu_ref);
        }
        double coeff = accum / (N * std_est * std_ref);
...
int main(int argc, char **argv) {
    ...
    // b. opencv's flow for validation
    ...
    for (int i = 0; i < kp2_multi_inverse.size(); i++) {
        if (success_multi_inverse[i]) {
            ...
            evaluator.Add(
                kp1.at(i).pt.x - kp2_multi_inverse.at(i).pt.x,
                D.GetPixelValue(kp1.at(i).pt.x, kp1.at(i).pt.y)
            );
        }
    }
    cout << "[Correlation Coefficient, Multi-Level Inverse]: " << evaluator.GetCoeff() << endl;
    ...
    for (int i = 0; i < pt2.size(); i++) {
        if (status[i]) {
            ...
            evaluator.Add(
                pt1.at(i).x - pt2.at(i).x,
                D.GetPixelValue(pt1.at(i).x, pt1.at(i).y)
            );
        }
    }
    cout << "[Correlation Coefficient, OpenCV]: " << evaluator.GetCoeff() << endl;

    cv::imshow("Tracked Multi Level, Forward", img2_multi_forward);
    cv::imwrite("disparity-map--multi-level-forward.png", img2_multi_forward);

    cv::imshow("Tracked Multi Level, Inverse", img2_multi_inverse);
    cv::imwrite("disparity-map--multi-level-inverse.png", img2_multi_inverse);

    cv::imshow("Tracked by OpenCV", img2_CV);
    cv::imwrite("disparity-map--opencv.png", img2_CV);
