...
def main(config):
    ...
    with open(
        os.path.join(config.input, "imu-calibration-results.json")
    ) as f:
    ...
    with open(
        os.path.join(config.input, "imu-calibration-results--summary.json"), 
        'w'
    ) as json_file:
    ...
    df_allan_variance_curve = pd.read_csv(
        os.path.join(config.input, "imu-calibration-results.csv")
    )
    ...
    plt.grid(True)
    plt.title('Allan Variance Curve, Gyroscope, VIO Simulated IMU')
    plt.savefig(
        os.path.join(config.input, "imu-calibration-results--gyro.png")
    )

    plt.figure(figsize=(16, 9), dpi=70)    
    plt.loglog(df_allan_variance_curve["T"], np.sqrt(df_allan_variance_curve["acc_avg"]))
    plt.grid(True)
    plt.title('Allan Variance Curve, Accelemeter, VIO Simulated IMU')
    plt.savefig(
        os.path.join(config.input, "imu-calibration-results--acc.png")
    )
