...
//    typedef std::unordered_map<unsigned long, std::shared_ptr<Vertex>> HashVertex;
     ...
     * @return
     */
    bool Solve(int iterations, const LambdaStrategy strategy);
...
private:
    ...
    /// 更新状态变量
    void UpdateStates(double alpha);

    void RollbackStates(double alpha); // 有时候 update 后残差会变大，需要退回去，重来
    ...
    void GetChiSquaredAndRho(double alpha, double &chi_squared, double &rho);
    bool IsGoodStepInLMNielsen(void);
    bool IsGoodStepInLMAlpha(void);
    bool IsGoodStepInLM(const LambdaStrategy strategy);
