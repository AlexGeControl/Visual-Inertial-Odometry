...
void CreatePointsLines(Points& points, Lines& lines)
{
    ...
    while(!f.eof())
    {
        ...
        if(!s.empty())
        {
            std::stringstream ss;
            // 
            // each line inside model definition consists of 2 points, which defines a line:
            // 
            ss << s;
            double x,y,z;
            ss >> x;
            ...
            //
            // for each definition point, check whether the point set contains it
            // if not, add to point set
            // 
