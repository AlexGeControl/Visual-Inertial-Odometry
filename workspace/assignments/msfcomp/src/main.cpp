...
{
    /*
    boost::regex format("^id\\s(\\d)+\\s(\\/\\w+\\s(\\w+|-*?\\d+|-*?\\d+\\.\\d+)\\s)+\\/;$");    
    
    std::cout << "Format:   \"" << format << "\"\n";
    std::cout << "Content:  \"" << content << "\"\n";

    if (boost::regex_match(content, format, boost::match_extra))
    {
        boost::smatch what;
        boost::regex_search(content, what, format);
    ...
    */
    boost::regex key_value("(id|\\/\\w+)\\s(\\w+|-*?\\d+|-*?\\d+\\.\\d+)\\s");
    boost::sregex_token_iterator iter(content.begin(), content.end(), key_value, 0);
